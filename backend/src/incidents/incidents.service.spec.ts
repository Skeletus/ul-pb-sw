...
describe('IncidentsService', () => {
  it('persists the alert, machine, site, and registering user relationship', async () => {
    ...
    await new IncidentsService(prisma as never).create(1, 5, { title: 'Idle', description: 'Waiting for material', severity: 'HIGH' as never });
  ...
  it('lists incidents associated with an alert', async () => {
    ...
    expect(await new IncidentsService(prisma as never).findByAlert(7)).toEqual([{ id: 1 }]);
  });
});
